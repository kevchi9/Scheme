...
(define cript   ;alfabeto criptato 
  (lambda (s)   ;s: stringa ; alfabeto da dare in input
    (lambda (k) ;intero compreso da 0 a 19
      (string-append (substring s k) (substring s 0 k))
      ))
  )
(define f (cript "ABCDEFGHILMNOPQRSTVX")) ; esempio (f 3) restituisce alfebto traslato di 3 posti
;mi da la posizione del carattere rispetto all'alfabeto originale

(define get-position   ; val: intero
...
(define find-cripted  ; val: intero
  (lambda (char k)
    (substring (f k) (- (get-position char (f 0)) 1) (get-position char (f 0)))
    )
  )
(define cript2 ; val : stringa criptata
  (lambda (k s); int k, valore della traslazione dell'alfabeto. String s frase da criptare
